...
import { isDebug } from './customHelpers';
import {
  serializeCanvas,
  serializeImageInline,
  serializeLinkInline,
  serializeMediaElements,
  serializeOption,
  serializeStyleInline,
  serializeUserFields,
} from './helpers/serialization/tags';
import type {
  DialogAttributes,
  KeepIframeSrcFn,
  MaskInputFn,
...
let canvasService: HTMLCanvasElement | null = null;
...
function serializeElementNode(
  ...
  switch (tagName) {
    case 'link':
      ...
      break;
    case 'style':
      if ((n as HTMLStyleElement).sheet) {
    ...
    case 'option':
      ...
      break;
    case 'dialog':
      if ((n as HTMLDialogElement).open) {
        ...
        (attributes as DialogAttributes).rr_open_mode = n.matches(
          'dialog:modal',
        )
          ? 'modal'
          : 'non-modal';
